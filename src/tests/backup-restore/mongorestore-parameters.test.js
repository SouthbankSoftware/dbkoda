 ...
 * @Last modified time: 2018-01-30T13:58:29+11:00
 *
...
describe('mongo restore test suite', () => {
  ...
  test('restore a server to verify parameter values', async () => {
    try {
      ...
      assert.equal(
        cmd,
        `mongorestore --host "localhost" --port "${mongoPort}" --db "${dbName}" --objcheck --oplogReplay --oplogLimit "10" --restoreDbUsersAndRoles --gzip --drop --dryRun --writeConcern "majority" --noIndexRestore --noOptionsRestore --keepIndexVersion --maintainInsertionOrder --numParallelCollections "5" --numInsertionWorkersPerCollection "3" --stopOnError --bypassDocumentValidation "data/test/dump"`
      );
    } catch (err) {
  ...
  test('restore a collection to verify its parameter values', async () => {
    try {
      const params = {
        [ParameterName.drop]: true,
        [ParameterName.pathInput]: 'data/test/dump bk',
        [ParameterName.dryRun]: true,
        [ParameterName.writeConcern]: 'majority',
        [ParameterName.noIndexRestore]: true,
        [ParameterName.noOptionsRestore]: true,
      ...
      assert.equal(await bkRestore.getParameterValue(ParameterName.drop), 'true');
      assert.equal(await bkRestore.getParameterValue(ParameterName.pathInput), 'data/test/dump bk');
      assert.equal(await bkRestore.getParameterValue(ParameterName.dryRun), 'true');
      assert.equal(await bkRestore.getParameterValue(ParameterName.writeConcern), 'majority');
      assert.equal(await bkRestore.getParameterValue(ParameterName.noIndexRestore), 'true');
      assert.equal(await bkRestore.getParameterValue(ParameterName.noOptionsRestore), 'true');
      ...
      assert.equal(
        cmd,
        `mongorestore --host "localhost" --port "${mongoPort}" --db "${dbName}" --collection "testcol" --objcheck --oplogReplay --oplogLimit "10" --restoreDbUsersAndRoles --gzip --drop --dryRun --writeConcern "majority" --noIndexRestore --noOptionsRestore --keepIndexVersion --maintainInsertionOrder --numParallelCollections "5" --numInsertionWorkersPerCollection "3" --stopOnError --bypassDocumentValidation "data/test/dump bk"`
      );
    } catch (err) {
      console.error('get error ', err);
      assert.fail(true, false);
