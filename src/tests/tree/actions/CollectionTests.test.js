...
describe('TreeAction:Collections', () => {
  ...
  test('Invoke create Collection', async () => {
    await r.treeAction
      .getTreeNodeByPath(['Databases', 'test'])
      .rightClick()
      .pause(500);
    await r.treeAction.clickContextMenu(r.createCLtemp.Title);
  });
  ...
  test('Enter Collection Properties', async () => {
    ...
    await r.treeAction.fillInDialogue(r.createCLtemp, r.createCLInput);
  });
  ...
  test('refresh', async () => {
    await r.treeAction._clickRefreshButton();
    await r.browser.pause(1000);
  });
  ...
  test('Insert some data', async () => {
    ...
    for (let i = 1; i < 5; i += 1) {
      cmd += sprintf(
        'db.getSiblingDB("%s").%s.insertOne({phone:%s});\n',
        'test',
        r.randomCollection,
        i
      );
  ...
  test('Insert from product', async () => {
    ...
    r.insertDocsData.DocumentArray =
      '[{a:10, y:1, c:1,phone:1}, {a:10, y:1, c:2,phone:1}, {a:10, y:1, c:3,phone:1}]';
    await r.treeAction
    ...
    await r.treeAction.fillInDialogue(r.insertDocsDDD, r.insertDocsData);
  ...
  test('Check that insert worked', async () => {
    const editorInput = sprintf(
      'var x=db.getSiblingDB("%s").%s.count();print("count="+x);\n\n',
      'test',
      r.randomCollection
    );
    if (debug) {
  ...
  test('Update from product', async () => {
    ...
    await r.treeAction.fillInDialogue(r.updateDocsDDD, r.updateDocsData);
  ...
  test('Check that update worked', async () => {
    ...
    const editorInput = sprintf(
      'var x=db.getSiblingDB("%s").%s.aggregate(' +
        stringPipeLine +
        ' ).next();print("sum="+x.ysum );\n\n',
      'test',
      r.randomCollection
    );
    if (debug) {
  ...
  test('Delete from product', async () => {
  ...
  test('Create index', async () => {
    ...
    await r.browser.pause(100);
    await r.treeAction.fillInDialogue(r.createIXtemp, r.createIXInput);
    if (debug) console.log('execute');
    await r.browser.pause(100);
    await r.treeAction.execute().pause(500);
    if (debug) console.log('close');
  ...
  test('Check that index is removed', async () => {
    const editorInput = sprintf(
      'db.getSiblingDB("%s").%s.getIndexes()',
      'test',
      r.randomCollection
    );
    const outputLines = await editorCommand(editorInput);
  ...
  test('Check that collection is removed', async () => {
    const editorInput = sprintf('db.getSiblingDB("%s").getCollectionNames()', 'test');
    const outputLines = await editorCommand(editorInput);
