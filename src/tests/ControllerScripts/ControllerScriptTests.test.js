...
describe('Mongoscripts:tests', () => {
  ...
  test('Check MongoScripts exist and have expected functions', async () => {
    const cmd =
      'var check=Object.keys(dbe).length>=16 & \
  ...
  test('Insert some data', async () => {
    ...
    for (let i = 1; i <= 10; i += 1) {
      cmd += sprintf(
        'db.getSiblingDB("%s").%s.insert({col_a:%s,col_b:%s,col_c:%s,col_d:%s,col_e:%s});\n',
        'test',
        r.randomCollection,
        i,
        i,
  ...
  test('Index recomendations for AND query ', async () => {
    const editorInput = sprintf(
      `
        ...
        print ('index AND tests OK',test1&test2&test3)`,
      'test',
      r.randomCollection
    );
    if (debug) {
  ...
  test('Index recomendations for OR query ', async () => {
    const editorInput = sprintf(
      `
        ...
        print ('index OR tests OK',test1&test2&test3&test4)`,
      'test',
      r.randomCollection
    );
    if (debug) {
  ...
  test("Don't recomend redundant index", async () => {
    const editorInput = sprintf(
      `
        ...
        print ('index redundant test',test1)`,
      'test',
      r.randomCollection,
      'test',
      r.randomCollection
