...
const downloadDrill = () => {
  return new Promise((resolve, reject) => {
    ...
    const drillTarFile = path.resolve(
      drillPath,
      `apache-${drillVersion}.tar.gz`,
    );
    const drillExtractedPath = path.resolve(
      drillPath,
      `apache-${drillVersion}`,
    );
    const options = {};
        ...
        tar
          ...
          .then(() => {
            ...
            sendStatusToRenderer(
              'updateDrillStatus',
              'COMPLETE',
              'drillCmd|' + drillExtractedPath,
            );
            resolve(drillExtractedPath);
          });
      });
      const progressUpdateFunc = _.throttle(
        (progress) => {
...
const downloadDrillController = () => {
  return new Promise((resolve, reject) => {
    ...
    if (!drillJavaControllerExists) {
      ...
      const ctrlProgressUpdateFunc = _.throttle(
        (progress) => {
